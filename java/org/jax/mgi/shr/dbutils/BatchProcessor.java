...
public class BatchProcessor
{
   ...
   * @throws DBException thrown if there is an error in the database
   */
  public int[] executeBatch() throws DBException, BatchException
  {
		if (isClosed)
		{
			DBExceptionFactory eFactory = new DBExceptionFactory();
    ...
    try {
    	int[] results = statement.executeBatch();
			if (logger != null)
			{
				...
				logger.logInfo(history.size() + " sql statements were run\n0 failed"); 
			}
      return results;
    }
    catch (BatchUpdateException e)
    {
      ...
      if (logger != null)
      {
      	logger.logError("The following is a list of all sql statements that " +
      	                "failed for this run");
      }

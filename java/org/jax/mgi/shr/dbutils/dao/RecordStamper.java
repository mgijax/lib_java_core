...
/**
 * @is a class for stamping XState classes by modifing their user and time
 * fields
 * @has a recordStampCfg for looking up job stream names within the
 * configuration
 * @does determines what type of record stamp is implemented by the given
 * XState class and modifies the appropriate fields for update and
 * insert operations in th database
 * @author mbw
...
public class RecordStamper {
   ...
   */
  public static void stampForInsert(RecordStampable rcd) throws
   ...
   * coniguration or an error looking up key values in the database
   */
  public static void stampForUpdate(RecordStampable rcd) throws
      RecordStampException {
    if (rcd instanceof RecordStampable_MGD)
   ...
   */
  private static void stampInsertMGD(RecordStampable_MGD rcd) throws
      RecordStampException {
    ...
    try {
      ...
      RecordStampException e2 = (RecordStampException)
          eFactory.getException(RecordStampExceptionFactory.UnknownUser, e);
   ...
   * configuration or looking up key values in the database
   */
  private static void stampInsertRADAR(RecordStampable_RADAR rcd) throws
      RecordStampException {
    Integer userKey = null;
    Timestamp t = new Timestamp(new Date().getTime());
    try {
   ...
   * configuration
   */
  private static void stampInsertMGDOrg(RecordStampable_MGDOrg rcd) throws
      RecordStampException {
    Timestamp t = new Timestamp(new Date().getTime());
   ...
   * configuration or looking up key values in the database
   */
  private static void stampUpdateMGD(RecordStampable_MGD rcd) throws
      RecordStampException {
    Timestamp t = new Timestamp(new Date().getTime());
    Integer userKey = null;
    try {
   ...
   * configuration
   */
  private static void stampUpdateMGDOrg(RecordStampable_MGDOrg rcd) throws
      RecordStampException {
    Timestamp t = new Timestamp(new Date().getTime());
    String modifiedBy = null;
    RecordStampCfg cfg = null;
